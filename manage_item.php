...
if(isset($_POST['delete_id'])) {
    ...
    // Redirect to same page to refresh item list
            ...
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        ...
                        <button type="button" class="btn btn-primary mt-2" data-bs-toggle="modal" data-bs-target="#editModal<?php echo $item['item_id']; ?>">Edit</button>
                    </div>
                </div>
            </div>
            <!-- Edit Modal -->
                        ...
                        <div class="modal-body">
                            <form action="" method="post">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="itemName" name="name" value="<?php echo $item['name']; ?>" required>
                                </div>
                                <div class="mb-3">
                                    <label for="productSelect" class="form-label">Product</label>
                                    <select class="form-select" id="productSelect" name="product_id" required>
                                        <?php foreach($products_dropdown as $product): ?>
                                            <option value="<?php echo $product['product_id']; ?>" <?php if($product['product_id'] == $item['product_id']) echo "selected"; ?>><?php echo $product['product_name']; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                <div class="mb-3">
                                    <label for="itemDescription" class="form-label">Description</label>
                                    <textarea class="form-control" id="itemDescription" name="description" rows="3"><?php echo $item['description']; ?></textarea>
                                </div>
                                <div class="mb-3">
                                    <label for="itemQuantity" class="form-label">Quantity</label>
                                    <input type="text" class="form-control" id="itemQuantity" name="quantity" value="<?php echo $item['quantity']; ?>">
                                </div>
                                <div class="mb-3">
                                    <label for="itemPrice" class="form-label">Price</label>
                                    <input type="text" class="form-control" id="itemPrice" name="price" value="<?php echo $item['price']; ?>">
                                </div>
